...
bool inPolygon(const Polygon &P, Point pt) {
...
Polygon polygonCut(Polygon P, Line l) {
    Polygon Q;
    for(int i = 0; i < P.size(); ++i) {
...
bool intersect1Point(Point a, Point b, Point c, Point d, Point &r) {
    double D = (b - a) % (d - c);
...
Polygon convexIntersect(Polygon P, Polygon Q) {
    ...
    do {
        ...
        if (intersect1Point(P[a1], P[a], Q[b1], Q[b], r)) {
...
double convexDiameter(Polygon pt) {
    const int n = pt.size();
    int is = 0, js = 0;
    for (int i = 1; i < n; ++i) {
